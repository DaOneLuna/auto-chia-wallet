...
async def generate_signed_spend_bundle(amount: uint64, puzzle_hash: bytes32, changeAddress: bytes32, coins: Set[Coin] = None, announcements: Set[Announcement] = None) -> Optional[SpendBundle]:
    spends = await _generate_unsigned_transaction(
        amount, puzzle_hash, uint64(0), None, coins, None, False, announcements, changeAddress
    )
    assert len(spends) > 0
